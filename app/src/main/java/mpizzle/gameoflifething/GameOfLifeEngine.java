...
import static mpizzle.gameoflifething.GameOfLifeView.ROWS;
import static mpizzle.gameoflifething.GameOfLifeView.COLUMNS;
import static mpizzle.gameoflifething.GameOfLifeView.PADDING;
        ...
        for (int i = 1; i <= columns; ++i) {
            for (int j = 1; j <= rows; ++j) {
                ...
                if ((currentGen[i][j] > 0 && (neighbours == 2 || neighbours == 3)) || (!(currentGen[i][j] > 0) && neighbours == 3)) {
                    nextGen[i][j] = (currentGen[i][j] != 16) ? ((currentGen[i][j] + 1) % 16) + 1 : 16;
                }
            }
        }
